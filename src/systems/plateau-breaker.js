...
function countWeeksWithoutHeavy(lift, entries8w, best) {
  const now = Date.now();
  for (let w = 1; w <= 8; w++) {
    const weekEntries = entries8w.filter(
      (e) => now - e.timestamp <= w * 7 * MS_PER_DAY && now - e.timestamp > (w - 1) * 7 * MS_PER_DAY
    );
...
function analyzeRPEPattern(lift, entries4w) {
  ...
  } else if (avgRPE >= 8.5 && rpe9PlusPct >= 0.4) {
    score = 70;
    evidence.push(
      `Average RPE ${avgRPE.toFixed(1)} with ${Math.round(rpe9PlusPct * 100)}% of sets at RPE 9+`
    );
  } else if (avgRPE >= 8.5) {
    score = 45;
...
export function diagnosePlateau(lift) {
  ...
  const entries4w = entries8w.filter((e) => now - e.timestamp <= WINDOW_4_WEEKS * MS_PER_DAY);

  // Get plateau context
  const recent4w = entries4w;
  const older4w = entries8w.filter((e) => now - e.timestamp > WINDOW_4_WEEKS * MS_PER_DAY);
...
function generateIntensityCycle(lift, tm) {
  ...
  return {
    durationWeeks: 4,
    summary:
      'Reintroduces intensity variation with ascending heavy work over 4 weeks, building to a test single.',
    weeks: [
      makeWeek(
        1,
        'Accumulation',
        ...
        7.0,
        acc,
        'Build a base with moderate volume'
      ),
      makeWeek(
        2,
        'Intensification',
        ...
        8.0,
        acc,
        'Introduce heavier doubles and singles'
      ),
      makeWeek(
        3,
        'Realization',
        ...
        8.5,
        acc,
        'Push to heavy singles near your max'
      ),
      makeWeek(
        4,
        'Test',
        ...
        9.0,
        acc,
        'Work up to a heavy single, then back off'
      ),
    ],
  };
}
...
function generateVolumeCycle(lift, tm) {
  ...
  return {
    durationWeeks: 4,
    summary:
      'Accumulates training volume with higher reps at moderate intensity, then transitions back to heavier work.',
    weeks: [
      makeWeek(
        1,
        'High Volume',
        ...
        6.0,
        acc,
        'Build work capacity with high reps'
      ),
      makeWeek(
        2,
        'Volume + Load',
      ...
      makeWeek(
        ...
        7.0,
        acc,
        'Reduced volume, heavier weights — feel the carryover'
      ),
    ],
  };
}
...
function generateFatigueCycle(lift, tm, diagnosis) {
  const acc = selectSmartAccessories(lift, 2);
  const interfering =
    diagnosis.analysisData.interferingLifts.map((l) => LIFT_NAMES[l]).join('/') || 'other lifts';
  return {
    durationWeeks: 3,
    ...
    weeks: [
      makeWeek(
        ...
        7.0,
        acc,
        `Maintain ${LIFT_NAMES[lift]}. Cut ${interfering} volume by 50%`
      ),
      makeWeek(
        2,
        'Recovery Push',
        ...
        7.5,
        acc,
        `Slight push on ${LIFT_NAMES[lift]}. ${interfering} at 65% volume`
      ),
      makeWeek(
        3,
        'Normalize',
        ...
        8.0,
        acc,
        'Return to normal across all lifts'
      ),
    ],
  };
}
...
function generateDeloadCycle(lift, tm) {
  ...
  return {
    durationWeeks: 3,
    summary:
      'Structured deload followed by a controlled ramp-up to let accumulated fatigue dissipate.',
    weeks: [
      makeWeek(
        1,
        'Deload',
        ...
        5.0,
        acc,
        'True deload — light and easy, focus on technique'
      ),
      makeWeek(
        2,
        'Re-acclimate',
      ...
      makeWeek(
        ...
        8.0,
        acc,
        'Push for a PR attempt — fatigue should be cleared'
      ),
    ],
  };
}
  ...
  return {
    ...
    weeks: [
      makeWeek(
        ...
        7.5,
        acc,
        'Main day as normal. Add a 2nd day at 65-70% for 3x6'
      ),
      makeWeek(
        2,
        'Build Pattern',
        ...
        8.0,
        acc,
        'Slight push on main day. Light day at 70% for 3x5'
      ),
      makeWeek(
        3,
        'Push',
        ...
        8.5,
        acc,
        'Main day peaks. Light day at 72% for 3x4'
      ),
      makeWeek(
        4,
        'Test',
        ...
        9.0,
        acc,
        'Work to a heavy single. Light day is optional recovery'
      ),
    ],
  };
}
  ...
  return {
    ...
    weeks: [
      makeWeek(
        ...
        7.5,
        acc,
        `Focus on ${weakPoint} accessories with higher volume`
      ),
      makeWeek(
        2,
        'Build',
        ...
        8.0,
        acc,
        'Push accessory weights up slightly'
      ),
      makeWeek(
        3,
        'Intensify',
        ...
        8.0,
        acc,
        'Heavier main work + maintained accessory volume'
      ),
      makeWeek(
        4,
        'Test',
        ...
        8.5,
        acc,
        'Test if the weak point work has paid off'
      ),
    ],
  };
}
