...
export function generateMesocycle(goal, model, durationWeeks, includeOptional) {
  ...
  LIFTS.forEach((l) => {
    baseTMs[l] =
      store.programConfig.trainingMaxes[l] || (bestE1RM(l) ? Math.round(bestE1RM(l) * 0.9) : 0);
  });
  ...
  for (let w = 1; w <= durationWeeks; w++) {
    ...
    const weekData = generateMesoWeek(
      w,
      durationWeeks,
      goal,
      model,
 ...
 */
export function generateMesoWeek(
  weekNum,
  totalWeeks,
  goal,
  model,
  ...
  if (isDeload) {
    ...
    if (progress < 0.4) {
      phase = 'Accumulation';
      pct = goalDef.pctRange[0];
      reps = goalDef.repRange[1];
      targetRPE = goalDef.rpeRange[0];
      numSets = goal === 'hypertrophy' ? 5 : 5;
    } else if (progress < 0.7) {
      phase = 'Intensification';
      pct = (goalDef.pctRange[0] + goalDef.pctRange[1]) / 2;
      reps = Math.round((goalDef.repRange[0] + goalDef.repRange[1]) / 2);
      targetRPE = (goalDef.rpeRange[0] + goalDef.rpeRange[1]) / 2;
      numSets = 4;
    } else {
      phase = 'Realization';
      pct = goalDef.pctRange[1];
      reps = goalDef.repRange[0];
      targetRPE = goalDef.rpeRange[1];
      numSets = 3;
    }
  }
...
export function recordMesocyclePerformance(session) {
  ...
  if (lastSet && typeof lastSet.reps === 'string' && lastSet.reps.includes('+')) {
    ...
    const rpeReduction =
      extraReps <= 2 ? extraReps * 0.5 : 1.0 + Math.log2(Math.max(1, extraReps - 1)) * 0.5;
...
export function adaptRemainingWeeks(lift) {
  ...
  const acwrIsHigh =
    liftFatigue &&
    (liftFatigue.status === 'red' || (!isPeaking && liftFatigue.status === 'yellow'));
  ...
  const weeksRemaining =
    store.activeMesocycle.durationWeeks - store.activeMesocycle.currentWeek + 1;
  ...
  if (shouldBlockIncrease) {
    ...
    adaptation = {
      type: 'blocked',
      pctChange: 0,
      reason: `RPE ${perf.actualRPE} below target but ACWR elevated — holding intensity`,
    };
    ...
    adaptation = {
      type: 'increase',
      pctChange: increase,
      reason: `RPE ${perf.actualRPE} well below target ${week.targetRPE}`,
    };
    for (
      let i = store.activeMesocycle.currentWeek - 1;
      i < store.activeMesocycle.weeks.length;
      i++
    ) {
    ...
    const decrease = 2.5;
    const reason =
      acwrTriggeredDecrease && rpeDiff < 1.5
  ...
  if (adaptation) {
    store.activeMesocycle.adaptationLog.push({
      ...
      adjustment:
        adaptation.type === 'blocked'
          ? 'Held (ACWR override)'
          : `${adaptation.type === 'increase' ? '+' : ''}${adaptation.pctChange}% intensity`,
      timestamp: Date.now(),
    });
    store.saveMesocycle();
  }
