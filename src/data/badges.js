...
export const BADGE_DEFINITIONS = [
  ...
  },
  {
    ...
    check: (ctx) => {
      const s = new Set();
      ctx.entries.forEach((e) => s.add(e.date));
      return s.size >= 50;
    },
  },
  {
  ...
  },
  {
    ...
    category: 'consistency',
    check: (ctx) => {
      const s = ctx.calcStreak();
      return s && s.longest >= 7;
    },
  },
  {
    id: 'all-three',
    name: 'Full SBD',
    desc: 'Log all 3 lifts in one session',
    icon: '\uD83C\uDFCB\uFE0F',
    category: 'consistency',
    check: (ctx) => {
      const sessions = ctx.groupSessions(ctx.entries);
      return sessions.some((s) => s.lifts.length >= 3);
    },
  },
  // Strength
  {
  ...
  },
  {
    id: '2x-deadlift',
    name: '2x BW Dead',
    desc: 'Deadlift 2x bodyweight',
    icon: '\uD83C\uDFCB\uFE0F',
    category: 'strength',
    check: (ctx) =>
      ctx.profile.bodyweight && ctx.bestE1RM('deadlift') >= ctx.profile.bodyweight * 2,
  },
  {
    id: '1plate',
    name: '1 Plate Club',
    desc: 'Hit 135 lbs on any lift',
    icon: '\uD83E\uDD49',
    category: 'strength',
  ...
  // Milestones
  {
    id: 'total-500',
    name: '500 Total',
    desc: 'Reach a 500 lb total',
    icon: '\uD83D\uDCAA',
    category: 'milestones',
    check: (ctx) => {
      const t = ctx.getTotal();
      return t && t >= 500;
    },
  },
  {
