...
import {
  STRENGTH_RATIO_BS_BALANCED,
  STRENGTH_RATIO_BS_WARNING,
  STRENGTH_RATIO_DS_BALANCED,
  STRENGTH_RATIO_DS_WARNING,
...
import {
  calcPriorWeekReview,
  calcWeeklyCoverage,
  calcAverageMuscleCoverage,
} from '../systems/weekly-coverage.js';
...
export function updateLiftCard(lift) {
  ...
  if (best !== null) {
    el.textContent = Math.round(displayWeight(best));
  ...
  if (bar) {
    if (store.goals[lift] && best) {
      bar.style.display = '';
      bar.querySelector('.goal-fill').style.width =
        Math.min(100, (best / store.goals[lift]) * 100) + '%';
    } else {
  ...
  if (cls) {
    badgeEl.textContent = clsShort[cls] || cls;
    badgeEl.className = 'class-badge ' + cls;
...
export function updateTotalCard() {
  ...
  if (total) {
    totalEl.textContent = Math.round(displayWeight(total));
  ...
  if (bar) {
    if (store.goals.total && total) {
      bar.style.display = '';
      bar.querySelector('.goal-fill').style.width =
        Math.min(100, (total / store.goals.total) * 100) + '%';
    } else {
      bar.style.display = 'none';
    }
  }
  ...
  if (total && store.profile.bodyweight) {
    tbw.textContent = (total / store.profile.bodyweight).toFixed(1) + 'x BW';
    tbw.style.display = '';
  } else {
  ...
  if (oc) {
    tBadge.textContent = ocShort[oc] || oc;
    tBadge.className = 'class-badge ' + oc;
...
export function updateStrengthRatios() {
  ...
  bsEl.className =
    'ratio-value ' +
    (bsRatio >= STRENGTH_RATIO_BS_BALANCED[0] && bsRatio <= STRENGTH_RATIO_BS_BALANCED[1]
      ? 'balanced'
      : bsRatio >= STRENGTH_RATIO_BS_WARNING[0] && bsRatio <= STRENGTH_RATIO_BS_WARNING[1]
        ? 'warning'
        : 'imbalanced');
  dsEl.textContent = Math.round(dsRatio) + '%';
  dsEl.className =
    'ratio-value ' +
    (dsRatio >= STRENGTH_RATIO_DS_BALANCED[0] && dsRatio <= STRENGTH_RATIO_DS_BALANCED[1]
      ? 'balanced'
      : dsRatio >= STRENGTH_RATIO_DS_WARNING[0] && dsRatio <= STRENGTH_RATIO_DS_WARNING[1]
        ? 'warning'
        : 'imbalanced');
}
...
export function updateScoreLine() {
  ...
  if (w && d) {
    ...
    if (cls)
      html += ` &middot; <span class="class-badge ${cls}" style="vertical-align:middle">${clsLabel[cls] || cls}</span>`;
    el.innerHTML = html;
  } else {
    el.style.display = 'none';
  }
...
export function updateFatigueBar() {
      ...
      .map((mg) => {
        ...
        const recBar =
          recPct !== null
            ...
            : '';
        return (
          `<div class="fatigue-card" data-muscle="${mg}">` +
  ...
  // Toggle button
  html +=
    `<div class="fatigue-cards-toggle" id="fatigue-cards-toggle">` +
  ...
  $('fatigue-row')
    .querySelectorAll('.fatigue-card[data-muscle]')
    .forEach((card) => {
...
export function updateStreakBar() {
  ...
  if (!streak || (streak.current === 0 && streak.longest <= 1)) {
    el.style.display = 'none';
    return;
  }
  el.style.display = 'flex';
  const icon =
    streak.current >= 7 ? '\uD83D\uDD25' : streak.current >= 3 ? '\u26A1' : '\uD83C\uDFCB\uFE0F';
...
export function updatePRStreakBar() {
  ...
  if (!store.dashboardWidgets.prStreak) {
    el.style.display = 'none';
    return;
  }
  const now = Date.now(),
...
export function renderPriorWeekCard() {
  ...
  if (gradeResult && !gradeResult.insufficient && gradeResult.grade) {
    const gradeColor =
      gradeResult.grade.startsWith('A') || gradeResult.grade.startsWith('B')
        ? 'var(--green)'
  ...
  if (insights && insights.chips.length > 0) {
    chipsHtml =
      ...
      insights.chips
        .map((c) => `<span class="insight-chip" style="background:${c.color}">${c.label}</span>`)
        .join('') +
      `</div>`;
  }

  el.innerHTML =
    `<div class="recap-card-header">Last Week in Review ${gradeHtml}</div>` +
...
function _compareRow(label, current, prior, pctChange) {
  ...
  const changeColor =
    pctChange > 0 ? 'var(--green)' : pctChange < 0 ? 'var(--red)' : 'var(--text-dim)';
  const changeStr =
    pctChange !== null && pctChange !== 0
      ? `<span style="color:${changeColor};font-size:0.65rem;font-weight:600">${arrow}${Math.abs(pctChange)}%</span>`
      : '';
...
function showPriorWeekSheet(review, gradeResult, insights) {
  ...
  if (review.prior) {
    ...
    const setsChange =
      p.totalSets > 0 ? Math.round(((review.totalSets - p.totalSets) / p.totalSets) * 100) : null;
    const volChange =
      p.totalVolume > 0
        ? Math.round(((review.totalVolume - p.totalVolume) / p.totalVolume) * 100)
        : null;
    ...
    html += _compareRow(
      'Volume',
      fmtNum(displayWeight(review.totalVolume)) + ' ' + store.unit,
      fmtNum(displayWeight(p.totalVolume)) + ' ' + store.unit,
      volChange
    );
    html += _compareRow(
      'Intensity',
      review.avgIntensity + '%',
      p.avgIntensity + '%',
      review.avgIntensity - p.avgIntensity
    );
  ...
  LIFTS.forEach((l) => {
    ...
    const prBadge =
      s.prs > 0
        ? ` <span style="color:var(--gold);font-size:0.65rem;font-weight:700">PR</span>`
        : '';
    ...
    const deltaStr =
      delta !== 0
        ? ` <span style="color:${delta > 0 ? 'var(--green)' : 'var(--red)'};font-size:0.6rem">${delta > 0 ? '+' : ''}${delta}</span>`
        : '';
  ...
  MUSCLE_GROUPS.forEach((mg) => {
    ...
    if (
      (status === 'Light' || status === 'Needs more' || status === 'Skipped') &&
      (fatigueStatus === 'red' || fatigueStatus === 'orange')
    ) {
      status = 'Recovering';
    }
    ...
    let vsAvgStr;
    if (vsAvg === null) {
      vsAvgStr = '<span style="color:var(--text-dim)">—</span>';
    } else if (vsAvg > 10) {
      vsAvgStr = `<span style="color:var(--green)">+${vsAvg}%</span>`;
    } else if (vsAvg < -10) {
      vsAvgStr = `<span style="color:var(--red)">${vsAvg}%</span>`;
    } else {
      vsAvgStr = `<span style="color:var(--text-dim)">${vsAvg > 0 ? '+' : ''}${vsAvg}%</span>`;
    }
  ...
  const mapContainer = document
    .getElementById('review-sheet-map')
    ?.querySelector('.body-map-container');
  if (mapContainer) {
    initBodyMapEvents(mapContainer, (mg) => {
      ...
      body.querySelector('.review-back-btn').onclick = () =>
        showPriorWeekSheet(review, gradeResult);
    });
  }
}
...
export function renderRecapCard() {
  ...
  if (!recap || (recap.sets === 0 && recap.prevSets === 0)) {
    el.style.display = 'none';
    return;
  }
  el.style.display = 'block';
  ...
  if (recap.prsThisWeek.length > 0)
    preview += ` &middot; ${recap.prsThisWeek.length} PR${recap.prsThisWeek.length > 1 ? 's' : ''}`;
  el.innerHTML =
    `<div class="recap-card-header">This Week</div>` +
...
function showRecapSheet(recap) {
  ...
  const volChangeStr =
    recap.volChange !== null
      ? `<div class="recap-stat-change ${recap.volChange >= 0 ? 'up' : 'down'}">${recap.volChange >= 0 ? '\u2191' : '\u2193'}${Math.abs(recap.volChange).toFixed(0)}%</div>`
      : '';
  const setsChangeStr =
    recap.setsChange !== null
      ? `<div class="recap-stat-change ${recap.setsChange >= 0 ? 'up' : 'down'}">${recap.setsChange >= 0 ? '\u2191' : '\u2193'}${Math.abs(recap.setsChange).toFixed(0)}%</div>`
      : '';
  ...
  LIFTS.forEach((l) => {
    ...
    const color = COLORS[l];
    const prBadge =
      prs > 0 ? ` <span style="color:var(--gold);font-size:0.65rem;font-weight:700">PR</span>` : '';
  ...
  MUSCLE_GROUPS.forEach((mg) => {
    ...
    const fatigueStatus =
      fatigueByMuscle && fatigueByMuscle[mg] ? fatigueByMuscle[mg].displayStatus : null;
    let status = data ? data.status : 'Skipped';
    if (
      (status === 'Light' || status === 'Needs more' || status === 'Skipped') &&
      (fatigueStatus === 'red' || fatigueStatus === 'orange')
  ...
  const mapContainer = document
    .getElementById('recap-sheet-map')
    ?.querySelector('.body-map-container');
  if (mapContainer) {
    initBodyMapEvents(mapContainer, (mg) => {
      const data = coverage[mg];
...
export function checkAndCelebrateMilestone() {
  ...
  let celebrated = {};
  try {
    celebrated = JSON.parse(localStorage.getItem(TOTAL_CELEBRATED_KEY)) || {};
  } catch (_e) {
    // noop — invalid JSON
  }
...
export function updateDashboard() {
  ...
  updateTotalCard();
  if (store.dashboardWidgets.ratios) updateStrengthRatios();
  else $('strength-ratios').style.display = 'none';
  updateScoreLine();
  if (store.dashboardWidgets.fatigue) updateFatigueBar();
  else $('fatigue-bar').style.display = 'none';
  if (store.dashboardWidgets.streak) updateStreakBar();
  else $('streak-bar').style.display = 'none';
  renderPriorWeekCard();
  if (store.dashboardWidgets.recap) renderRecapCard();
  else $('recap-card').style.display = 'none';
  updatePRStreakBar();
  updatePlateauCards();
