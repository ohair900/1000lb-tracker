...
export function showFatigueDetail(mg) {
  ...
  if (!detail) {
    html +=
      '<div style="padding:24px 0;text-align:center;color:var(--text-dim)">Not enough data for ' +
      mg +
      ' (need 3+ entries in 28 days)</div>';
  ...
  else if (detail.status === 'yellow' && recoveryPct !== null && recoveryPct < 0.15)
    displayStatus = 'red';
  else if (recoveryPct !== null && recoveryPct < 0.15) displayStatus = 'red';
  else if (detail.status === 'yellow' && recoveryPct !== null && recoveryPct < 0.4)
  ...
  if (recoveryPct !== null) {
    ...
    const lastStr =
      hrsAgo !== null ? (hrsAgo < 24 ? `${hrsAgo}h ago` : relativeDay(detail.lastTs)) : 'N/A';
    html += `<div class="sheet-section" style="--i:${sectionIdx++}">`;
    html +=
      `<div class="fatigue-detail-banner ${displayStatus}">` +
