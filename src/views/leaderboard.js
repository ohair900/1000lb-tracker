...
export async function renderLeaderboard() {
  ...
  if (!currentUser) {
    container.innerHTML =
      '<div class="empty-state" style="color:var(--text-dim);text-align:center;padding:32px 0">Sign in to view the leaderboard.</div>';
  ...
  container.innerHTML =
    '<div style="color:var(--text-dim);text-align:center;padding:32px 0">Loading...</div>';

  // Fire both fetches in parallel — leaderboard is required, crews are best-effort
  const [data, crews] = await Promise.all([fetchLeaderboard(), fetchUserCrews().catch(() => [])]);
...
function renderStrengthTab() {
  ...
  const controls = `<div class="lb-controls-row">
  ...
  // Apply filters
  const now = Date.now();
  let pool = getPool().filter(
    (e) => (e[sort === 'wilks' ? 'wilks' : sort === 'dots' ? 'dots' : sort] || 0) > 0
  );
  if (wc) pool = pool.filter((e) => e.weightClass === wc);
  if (activeOnly)
    pool = pool.filter((e) => e.lastTrainedAt && now - e.lastTrainedAt <= 7 * MS_PER_DAY);

  const sorted = [...pool].sort((a, b) => (b[sort] || 0) - (a[sort] || 0));

  if (sorted.length === 0) {
    $('leaderboard-list').innerHTML =
      `<div class="empty-state" style="color:var(--text-dim);text-align:center;padding:32px 0">No lifters match these filters.</div>`;
    return;
  }
  ...
  sorted.forEach((entry, i) => {
    ...
    const valueLabel =
      sort === 'wilks' || sort === 'dots'
        ? entry[sort] != null
          ? entry[sort].toFixed(1)
          : '—'
        : fmt(entry[sort] || 0);
    const hotStreak = _isHotStreak(entry) ? '<span class="lb-hot-chip">🔥</span>' : '';
    const wcLabel = entry.weightClass
      ? ` <span class="lb-wc-label">${entry.weightClass}${entry.weightClass.endsWith('+') ? '' : 'kg'}</span>`
      : '';
...
function renderStreaksTab() {
  ...
  if (sorted.length === 0) {
    $('leaderboard-list').innerHTML =
      `<div class="empty-state" style="color:var(--text-dim);text-align:center;padding:32px 0">No active streaks yet.</div>`;
    return;
  }
...
function renderImprovedTab() {
  ...
  const pool = getPool().filter(
    (e) => (e.totalAt30dAgo || 0) > 0 && (e.total || 0) > (e.totalAt30dAgo || 0)
  );
  ...
  if (withGain.length === 0) {
    $('leaderboard-list').innerHTML =
      `<div class="empty-state" style="color:var(--text-dim);text-align:center;padding:32px 0">No improvement data yet — check back in a few weeks.</div>`;
    return;
  }
...
function renderHallTab() {
  ...
  Object.values(byMilestone).forEach((arr) =>
    arr.sort((a, b) => (b._achievedAt || '').localeCompare(a._achievedAt || ''))
  );
  ...
  if (!anyShown) {
    $('leaderboard-list').innerHTML =
      `<div class="empty-state" style="color:var(--text-dim);text-align:center;padding:32px 0">No milestones reached yet.</div>`;
    return;
  }
  $('leaderboard-list').innerHTML = html;
...
function _renderCompare(me, them) {
  ...
  lifts.forEach((lift) => {
    ...
    const gapStr =
      gap === 0
        ? ''
        : gap > 0
          ? `<span class="lb-cmp-gap-up">+${fmt(gap)}</span>`
          : `<span class="lb-cmp-gap-down">${fmt(gap)}</span>`;
  ...
  const totalGapStr =
    totalGap === 0
      ? ''
      : totalGap > 0
        ? `<span class="lb-cmp-gap-up">+${fmt(totalGap)}</span>`
        : `<span class="lb-cmp-gap-down">${fmt(totalGap)}</span>`;
...
export function initLeaderboardTab() {
  ...
  $('crew-create-close').addEventListener(
    'click',
    () => ($('crew-create-modal').style.display = 'none')
  );
  $('crew-join-close').addEventListener(
    'click',
